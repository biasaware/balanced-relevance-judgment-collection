...
def calculate_doc_score(doc_str, parse):
    ...
    :return:
    """
    liwc_attributes = ['affect','posemo', 'negemo', 'anx', 'anger' , 'sad','cogproc','insight', 'cause', 'discrep', 'tentat', 'certain', 'differ', 'drives', 'affiliation','achiev', 'power', 'reward', 'risk', 'work','leisure', 'home', 'money', 'relig', 'death']
    ...
    for token in doc_tokens:
        ...
        for category in parse(token):
            ...
            if category in persconc:
                persconc_count += 1
                
    liwc_counts = Counter(categories)
    persconc_value = persconc_count / token_counter
    ...
    attributes_existance.insert(19, persconc_value)
...
def main():
  ...
  searcher = SimpleSearcher('/msmarco-passage/lucene-index-msmarco') #path to msmarco index
  ...
  df = pd.DataFrame(liwc_score_total, columns = ['cutoff','affect','posemo', 'negemo', 'anx', 'anger' , 'sad','cogproc','insight', 'cause', 'discrep', 'tentat', 'certain', 'differ', 'drives', 'affiliation','achiev', 'power', 'reward', 'risk', 'persconc', 'work','leisure', 'home', 'money', 'relig', 'death'])
